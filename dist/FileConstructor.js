...
var File, Path, Promise, SimplyImport, chalk, eventsLog, exec, fileInstances, fs, getFile, regEx, watcher;
...
File = function(filePath1, watchContext1, options1, eventType) {
...
File.prototype.getExtension = function() {
  var extension, i, len, path, pathsToTry;
  extension = Path.extname(this.filePath);
  if (extension) {
    return extension;
  } else {
    pathsToTry = [this.filePath + ".js", this.filePath + ".coffee", this.filePath + ".sass", this.filePath + ".scss"];
...
File.prototype.process = function(eventType) {
  if (!(eventType === 'scan' || !eventType)) {
    eventsLog.add(chalk.bgGreen.bgGreen.black(eventType) + ' ' + chalk.dim(this.filePathShort));
  }
...
module.exports = getFile = function(filePath, watchContext, options, eventType) {
  var ref;
  return ((ref = fileInstances[filePath]) != null ? ref.process(eventType) : void 0) || (fileInstances[filePath] = new File(filePath, watchContext, options, eventType));
};
